...
from pyecharts.charts import Map, Page, Line, Pie
...
def  chinaPip():
    ...
    ws_china_city = wb['中国省份疫情数据']
    ws_china_confirmed.delete_rows(1)
    ws_china_crued.delete_rows(1)
    ws_china_died.delete_rows(1)
    ws_china_city.delete_rows(1)

    y_city_confirm = []
    y_city_label = []

    for city_confirm in ws_china_city.values:
        y_city_label.append(city_confirm[0])
        y_city_confirm.append(city_confirm[2])
    ...
    pie = (
        Pie()
            ...
            "",
            [list(z)
             for z in zip(y_city_label, y_city_confirm)],
            radius=["30%", "75%"],
            center=["25%", "50%"],
            rosetype="radius",
